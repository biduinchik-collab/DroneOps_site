    ...
    <div className="interface-shell relative overflow-hidden rounded-lg border border-steel/20 bg-obsidian shadow-glow">
      <div className="grid min-h-[520px] lg:grid-cols-[1.15fr_0.85fr]">
        <div className="relative border-b border-steel/20 bg-void lg:border-b-0 lg:border-r">
          ...
          <div className="relative z-10 flex items-center justify-between border-b border-steel/20 px-5 py-4">
          ...
          <div className="relative h-[430px]">
            ...
            >
              ...
              <path
                ...
                d="M92 354 C164 270 210 286 300 212 C382 144 470 156 624 74"
                fill="none"
                pathLength={1}
                stroke="rgba(26,143,209,0.9)"
                strokeWidth="3"
              />
              <path
              ...
              <path className="route-dash" d="M164 292 L282 240 L408 268 L548 196" fill="none" stroke="rgba(237,240,244,0.34)" strokeDasharray="6 9" strokeWidth="1.5" />
              {[
                [92, 354],
                [300, 212],
                [624, 74],
        ...
        <div className="relative z-10 bg-panel/70 p-5 sm:p-7">
          <div className="grid gap-4">
            {preview.panels.map((panel, index) => (
              <div key={panel.label} className="preview-panel rounded-md border border-steel/20 bg-void/60 p-4" style={panelDelay(index)}>
            ...
            <ul className="mt-4 grid gap-3">
              {preview.checklist.map((item, index) => (
                <li key={item} className="flex gap-3 text-sm font-light leading-6 text-mist">
                  <span className="check-dot mt-2 h-1.5 w-1.5 flex-none rounded-full bg-signal" style={panelDelay(index)} />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
...
function nodeDelay(index: number): CSSProperties {
  return { "--node-delay": `${500 + index * 140}ms` } as CSSProperties;
}
